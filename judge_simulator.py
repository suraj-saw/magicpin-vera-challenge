...
LLM_API_KEY = os.environ.get("LLM_API_KEY", "")  # <-- Or set in .env

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = os.environ.get("LLM_MODEL", "")  # <-- Or set in .env
...
def print_success(text: str):
    # Using green for passing tests to make it easy to spot in a sea of terminal text
    print(f"{Colors.GREEN}[PASS]{Colors.RESET} {text}")

def print_fail(text: str):
    # Red for failures so they immediately jump out
...
class LLMProvider(ABC):
    ...
    # Decided to abstract this so we can easily swap between OpenAI, Anthropic, Gemini etc. 
    # without rewriting the core judging logic.
    @abstractmethod
...
class MockProvider(LLMProvider):
    def __init__(self, model: str = ""):
        self.model = model or "mock-evaluator"

    def name(self) -> str:
        return f"Mock ({self.model})"

    def complete(self, prompt: str, system: str = None) -> str:
        if "Score each dimension" in prompt or "Analyze this candidate message" in str(system):
            return json.dumps({
                "specificity": 9,
                "specificity_reason": "Contains exact numbers, dates, and citations",
                "category_fit": 9,
                "category_fit_reason": "Clinical and professional vocabulary used appropriately",
                "merchant_fit": 9,
                "merchant_fit_reason": "Honors merchant identity and language preference",
                "decision_quality": 9,
                "decision_quality_reason": "Trigger urgency and timing clearly justified",
                "engagement_compulsion": 9,
                "engagement_reason": "Strong curiosity hook and clear binary CTA",
                "hint": "Mock evaluation passed successfully"
            })
        return "Yes please, let's do it and proceed with the setup."
...
class DatasetLoader:
    ...
    def load(self) -> bool:
        try:
            # We load everything from the local dataset folder to keep it self-contained.
            # Avoids needing a database just for running the judge locally.
...
class BotClient:
    ...
    def _request(self, method: str, path: str, timeout: int = 30,
        ...
        # A lightweight wrapper around urllib to avoid needing the 'requests' library.
        # Returns (parsed_json, error_string, latency_ms).
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
    ...
    def _parse_response(self, response: str, action: Dict) -> ScoreResult:
        ...
        # LLMs are notoriously bad at outputting pure JSON without markdown blocks (like ```json), 
        # so we use a regex search to extract just the curly brace block.
    ...
    def _fallback_score(self, action: Dict) -> ScoreResult:
        ...
        # If the LLM output is garbled or we hit an error, we fall back to a simple heuristic.
        # Just count the numbers in the message to give a baseline specificity score.
...
class JudgeSimulator:
    ...
    def run(self, scenario: str) -> bool:
        ...
        # Dispatch table for scenarios. Let's us easily add new test cases later.
